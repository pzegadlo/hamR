...
BHoptim_max <- function(calib_vec, preset_vec, calib_names, preset_names, obs, burnin, tru, h, bestlik, N=1) {
...
foreach(s = 1:30) %do% {
  ...
  y <- foreach(cnt = 1:30, .packages= c("pso")) %dorng% {
    ...
    psoptim(rep(NA,ncalib), BHoptim_max, calib_vec, preset_vec, calib_names, preset_names, obs, burnin, tru, h, bestlik, N, control = list(maxf=3000, vectorize=FALSE))
